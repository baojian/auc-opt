...
def fetched_data():
    ...
    if not os.path.exists('/data/path/%s/' % data_set):
        os.mkdir('/data/path/%s/' % data_set)
    f_w = open('/data/path/%s/raw_%s' % (data_set, data_set), 'w')
...
def generate_seismic():
    ...
    f_w = open('/data/path/%s/raw_%s' % (data_name, data_name), 'w')
    f_w.write(str(len(x_tr)) + ' ' + str(len(x_tr[0])) + '\n')
    for sample, label in zip(x_tr, y_tr):
        for ind, val in zip(range(len(sample)), sample):
            f_w.write(str(ind) + ':' + str(val) + ' ')
