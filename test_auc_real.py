...
def cmd_svm_perf(sub_x_tr, sub_y_tr, sub_x_te, sub_y_te, para_xi, kernel):
    ...
    file_pred = tempfile.NamedTemporaryFile()
    if kernel == 'linear':
        learn_cmd = "./svm_perf/svm_perf_learn -v 0 -y 0 -w 9 -t 0 -c %.10f --b 1 %s %s"
        os.system(learn_cmd % (para_xi, f_tr.name, file_model.name))
        file_model.seek(0)
    else:
        learn_cmd = "./svm_perf/svm_perf_learn -v 0 -y 0 -w 3 --i 0 -t 2 -c %.10f --b 0 --k 500 %s %s"
        os.system(learn_cmd % (para_xi, f_tr.name, file_model.name))
        file_model.seek(0)
    pred_cmd = "./svm_perf/svm_perf_classify -v 0 -y 0  %s %s %s"
    os.system(pred_cmd % (f_tr.name, file_model.name, file_pred.name))
...
def run_algo_spam_l2(para):
    ...
    method = 'spam'
    num_passes, step_len, verbose, record_aucs, stop_eps, k_fold = 100, 1e8, 0, 0, 1e-6, 5
    ...
    for (ind_xi, para_xi), (ind_l2, para_l2) in product(enumerate(list_c), enumerate(list_l2)):
        ...
        for ind, (sub_tr_ind, sub_te_ind) in enumerate(kf.split(np.zeros(shape=(len(x_tr), 1)))):
            ...
            auc = 0.0 if (np.isnan(wt).any() or np.isfinite(wt).any()) \
                else roc_auc_score(y_true=sub_y_te, y_score=np.dot(sub_x_te, wt))
    ...
    wt = np.zeros(data['p']) if (np.isnan(wt).any() or np.isfinite(wt).any()) else wt
...
def run_algo_adaboost(para):
    ...
    for (ind_xi, n_est) in enumerate(list_n_est):
        kf = KFold(n_splits=k_fold, shuffle=False)  # Folding is fixed.
        for ind, (sub_tr_ind, sub_te_ind) in enumerate(kf.split(np.zeros(shape=(len(x_tr), 1)))):
            adaboost = AdaBoostClassifier(base_estimator=None, n_estimators=n_est,
                                          learning_rate=.1, algorithm='SAMME.R', random_state=trial_id)
            sub_x_tr, sub_y_tr, sub_x_te, sub_y_te = get_standard_data_sub(data, trial_id, sub_tr_ind, sub_te_ind)
    ...
    adaboost = AdaBoostClassifier(base_estimator=None, n_estimators=best_n_est,
                                  learning_rate=.1, algorithm='SAMME.R', random_state=trial_id)
    ...
    for (ind_xi, para_xi) in enumerate(list_c):
        ...
        for ind, (sub_tr_ind, sub_te_ind) in enumerate(kf.split(np.zeros(shape=(len(x_tr), 1)))):
            rbf_svm = SVC(C=para_xi, kernel='rbf', degree=3, gamma='scale',
                          ...
                          tol=1e-5, cache_size=5000, class_weight=class_weight,
...
def run_algo_lr(para):
    ...
    start_tr_time = time.time()
    lr = LogisticRegression(
        penalty='l2', dual=False, tol=1e-5, C=best_c, fit_intercept=True,
        intercept_scaling=1, class_weight=class_weight, random_state=trial_id,
        solver='lbfgs', max_iter=5000, multi_class='auto', verbose=0,
        warm_start=False, n_jobs=None, l1_ratio=None)
...
def run_algo_rf(para):
    ...
    for (ind_xi, n_est) in enumerate(list_n_est):
        ...
        for ind, (sub_tr_ind, sub_te_ind) in enumerate(kf.split(np.zeros(shape=(len(x_tr), 1)))):
            rand_forest = RandomForestClassifier(
                n_estimators=n_est, criterion="gini", max_depth=None, min_samples_split=2, min_samples_leaf=1,
    ...
    best_n_est = list_n_est[int(np.argmax(np.mean(auc_matrix, axis=1)))]
    rand_forest = RandomForestClassifier(
        n_estimators=best_n_est, criterion="gini", max_depth=None, min_samples_split=2, min_samples_leaf=1,
        min_weight_fraction_leaf=0., max_features="auto", max_leaf_nodes=None, min_impurity_decrease=0.,
...
def run_algo_gb(para):
    ...
    grad_boost = GradientBoostingClassifier(
        ...
        warm_start=False, presort='deprecated', validation_fraction=0.1, n_iter_no_change=None, tol=1e-4,
        ccp_alpha=0.0)
...
def parallel_by_method_dataset(dataset, method, num_cpus):
    ...
    pkl.dump(reduce(lambda a, b: {**a, **b}, results_pool),
             open(root_path + 'results/real/results_%s_%s.pkl' % (dataset, method), 'wb'))


def main():
    if sys.argv[1] == 'single':
        parallel_by_method_dataset(dataset=sys.argv[2], method=sys.argv[3], num_cpus=int(sys.argv[4]))
    elif sys.argv[1] == 'part':
        list_method = ['rank_boost', 'adaboost', 'c_svm', 'b_c_svm', 'lr', 'b_lr', 'rf', 'b_rf', 'gb', 'spam']
        for method in list_method:
            parallel_by_method_dataset(dataset=sys.argv[2], method=method, num_cpus=int(sys.argv[3]))
    elif sys.argv[1] == 'all':
